...
void main()                                                                     \n\
{                                                                                \n\
    gl_Position = vec4(vPosition.x + offset.x, vPosition.y + offset.y, vPosition.z, 1.0);  \n\
	color = colorOffset;							\n\
}";
...
void display(){
	...
	int uniformOffsetID = glGetUniformLocation(shaderProgramID, "offset");
	int colorOffsetID = glGetUniformLocation(shaderProgramID, "colorOffset");
	for (int i = 0; i < 4; i++) {
		glBindVertexArray(triVAO);
		glUniform2f(uniformOffsetID, ((double)rand() / (RAND_MAX)) * 1.6f - 0.8f, ((double)rand() / (RAND_MAX)) * 1.6f - 0.8f);
		glUniform4f(colorOffsetID, ((double)rand() / (RAND_MAX)), ((double)rand() / (RAND_MAX)), ((double)rand() / (RAND_MAX)), ((double)rand() / (RAND_MAX)));
		
		glDrawArrays(GL_TRIANGLES, 0, 3);

		glBindVertexArray(squVAO);
		glUniform2f(uniformOffsetID, ((double)rand() / (RAND_MAX)) * 1.6f - 0.8f, ((double)rand() / (RAND_MAX)) * 1.6f - 0.8f);
		glUniform4f(colorOffsetID, ((double)rand() / (RAND_MAX)), ((double)rand() / (RAND_MAX)), ((double)rand() / (RAND_MAX)), ((double)rand() / (RAND_MAX)));
...
void init()
{
	...
	GLfloat vertices[] = {-0.2f, -0.2f, 0.0f,
			0.2f, -0.2f, 0.0f,
			0.0f, 0.2f, 0.0f};
	...
	GLfloat squareVertices[] = {-0.1f, -0.1f, 0.0f,
		0.1f, -0.1f, 0.0f,
		0.1f, 0.1f, 0.0f,
		-0.1f, -0.1f, 0.0f,
		0.1f, 0.1f, 0.0f,
	...
	GLuint squVBO;
	glGenVertexArrays(1, &squVAO);
	glBindVertexArray(squVAO);
	glGenBuffers(1, &squVBO);

	glBindBuffer(GL_ARRAY_BUFFER, squVBO);
	// After binding, we now fill our object with data, everything in "Vertices" goes to the GPU
